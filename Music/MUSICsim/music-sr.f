...
*! Sea water parameters (density and cross-sections) are used instead
*! of those for pure water
*!
...
*      PRINT *,'music: loading energy losses table...'
      OPEN(UNIT=4,file='music-eloss-sr.dat',
...
*      PRINT *,'music: loading integral cross sections...'
      OPEN(UNIT=1,file='music-cross-sections-sr.dat',
     *  FORM='formatted',STATUS='old')
      READ(1,3)CS2
...
***********************************************************************
     ...
     *(x0,y0,z0,the0,ph0,Emuin0,depth0,rh0,tmu0)       ! changed 
      implicit real*8 (a-h,o-z)
      REAL*8 n_a,n_z,n_rho,n_lambda
      COMMON/rock2/n_a,n_z,n_rho,n_lambda
      ...
      n_rho=rh0         ! added 
        ...
        gamma=(Emu_0+Emu_f)/2./0.105655
        beta=dsqrt(gamma*gamma-1.)/gamma
        vmu=beta*29.9792458
        tmu=dr/vmu/n_rho
        tmu0=tmu0+tmu
        
        call coord_transform(x1,y1,z1,x,y,z,the0,ph0)  ! changed 
        x0=x1/n_rho+x0
        y0=y1/n_rho+y0
        z0=z1/n_rho+z0

        call angle_transform(the0,ph0,theta,phi)       ! changed 
...
C               AND IONISATION)
	...
	implicit real*8 (a-h,o-z)
      ...
      PARAMETER (mmu=0.105655d0)
      PARAMETER (ms_flag=1)   !to switch off mult.scatt. put ms_flag=0
      PARAMETER (d_flag=1)    !to switch off scatt. due to other processes
                              ! put d_flag=0
